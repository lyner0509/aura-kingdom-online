...
export const ITEMMALL_CATEGORY_LABELS: Record<number, string> = {
  1: 'Rekomendasi & Hot',
  2: 'Kostum & Fashion',
  3: 'Eidolon & Mitra',
  4: 'Batu Permata & Gem',
  ...
  47: 'Event Khusus',
  48: 'Populer & Terlaris',
  49: 'Paket Spesial & Boost',
  51: 'Koin & Token',
  52: 'Rainbow Coin',
  99: 'Paket Promo',
};

export function ItemMallPage({ onDirtyChange }: { onDirtyChange: (dirty: boolean) => void }) {
  ...
  async function load() {
    ...
    setError('');
    try {
      const result = await api.itemMall();
      setData(result);
      setRows(result.rows);
      ...
      setError(e instanceof Error ? e.message : 'Data Item Mall gagal dimuat.');
    } finally {
      setBusy(false);
    }
  }
  ...
  function edit(row: ItemMallItem, field: Editable, value: number) {
    setNotice('');
    setRows(current => current.map(item => item === row ? { ...item, [field]: value } : item));
  }

  function remove(row: ItemMallItem) {
  ...
  async function save() {
    ...
    setNotice('');
    try {
      const result = await api.saveItemMall(data.revision, rows);
      setData({ ...data, rows: structuredClone(rows), revision: result.revision });
      setNotice(result.changed
        ? 'Perubahan Item Mall berhasil disimpan ke database game.'
        : 'Tidak ada perubahan untuk disimpan.');
      try {
        const latest = await api.itemMall();
        setData(latest);
        setRows(latest.rows);
  ...
  return (
    <div className="loyalty-editor itemmall-editor">
      <section className="panel">
        <header className="loyalty-header itemmall-header">
          <div>
            <p className="kicker">Pengaturan admin</p>
            <h3>Item Mall (AP Store)</h3>
            <p>Kelola katalog barang toko AP, kategori tab, harga Aeria Points (AP), diskon promo, dan batas pembelian.</p>
          </div>
          <div className="loyalty-actions itemmall-actions">
            <button
              type="button"
        ...
        {showAddForm && (
          <div className="loyalty-add-box">
            <h4>Tambah Item Baru ke Item Mall</h4>
                ...
                >
                  {groups.map(g => (
                    <option key={g} value={g}>
                      {ITEMMALL_CATEGORY_LABELS[g] || `Grup ${g}`} (Grup {g})
                    </option>
                  ))}
                </select>
              </label>
              ...
              <select value={selectedGroup} onChange={e => setSelectedGroup(e.target.value)}>
                ...
                {groups.map(g => (
                  <option key={g} value={String(g)}>
                    {ITEMMALL_CATEGORY_LABELS[g] || `Grup ${g}`} ({rows.filter(r => r.item_group === g).length})
                  </option>
                ))}
              </select>
            </label>
        ...
        {!data ? (
          <div className="empty">{busy ? 'Memuat Item Mall…' : 'Data belum tersedia. Klik muat ulang.'}</div>
        ) : !rows.length ? (
          <div className="empty">Belum ada item di Item Mall. Klik "Tambah Item" untuk menambahkan.</div>
        ) : !filteredRows.length ? (
              ...
              <table className="loyalty-table">
                <thead>
                  <tr>
                    ...
                    <th style={{ width: '110px' }}>Harga AP</th>
                    <th style={{ width: '110px' }}>Diskon AP</th>
                      ...
                      <tr key={slotKey} className={row.sell === 0 ? 'row-inactive' : ''}>
                        ...
                        <td>
                          <span className="category-badge">
                            {ITEMMALL_CATEGORY_LABELS[row.item_group] || `Grup ${row.item_group}`}
                          </span>
                        </td>
                        <td>
                          <input
              ...
              <div>
                ...
                <p>
                  {invalid
                    ...
                    : 'Perubahan akan disimpan langsung ke tabel public.itemmall (money_unit = 1) dan dicatat di riwayat audit.'}
                </p>
              </div>
              <button
                ...
                onClick={save}
              >
                {busy ? 'Menyimpan…' : 'Simpan Perubahan Item Mall'}
              </button>
            </div>
          </>
        )}
      ...
      {data && (
        <section className="panel loyalty-history">
          <header>
            <h3>Riwayat Perubahan Item Mall</h3>
          </header>
          {data.history.length ? (
            <ul>
