...
export function ParagonPage({ onDirtyChange }: { onDirtyChange: (dirty: boolean) => void }) {
  ...
  async function load() {
    ...
    try {
      const result = await api.paragon(); setData(result); setRows(result.rows);
  ...
  async function save() {
    ...
    try {
      ...
      try { const latest = await api.paragon(); setData(latest); setRows(latest.rows); } catch { /* The save already succeeded. */ }
  ...
  return <div className="paragon-editor">
    <section className="panel">
      ...
      {!data ? <div className="empty">{busy ? 'Memuat Paragon Table…' : 'Tabel belum tersedia. Coba muat ulang.'}</div> : !rows.length ? <div className="empty">Belum ada hadiah Paragon di server.</div> : <>
        ...
        <div className="table-wrap"><table className="paragon-table"><thead><tr><th>Slot</th><th>Item ID</th><th>Jumlah</th><th>Peluang (%)</th>{flags.map(([key, label]) => <th key={key}>{label}</th>)}</tr></thead>
          <tbody>{visible.map(row => <tr key={`${row.category}/${row.weekday}/${row.drop_level}/${row.level_order}`}><td><strong>{row.level_order}</strong></td>
            {(['item_id', 'max_stack', 'drop_rate'] as const).map(field => <td key={field}><input aria-label={`${field === 'item_id' ? 'Item ID' : field === 'max_stack' ? 'Jumlah' : 'Peluang'} slot ${row.level_order}`} type="number" min={field === 'drop_rate' ? 0 : 1} max={field === 'drop_rate' ? 100 : field === 'max_stack' ? 32767 : 2147483647} step={field === 'drop_rate' ? 'any' : 1} disabled={busy || data.readOnly} value={Number.isNaN(row[field]) ? '' : row[field]} onChange={e => edit(row, field, e.target.value === '' ? NaN : Number(e.target.value))}/></td>)}
